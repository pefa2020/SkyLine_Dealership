...
import "../styles/ManagerAddCar.css";

function ManagerAddCar() {
  ...
  const [vin_check, setVinCheck] = useState("");
  const [make, setMake] = useState("");
  const [model, setModel] = useState("");
  const [year, setYear] = useState("");

  const handleVinCheck = (event) => {
    ...
    if (
      ...
      model != "" &&
      year != "" &&
      regex_year.test(year)
    ) {
      const jsonData = { vin: vin_check, make: make, model: model, year: year };
      const url = "http://localhost:5000/insertCarInInv";
      fetch(url, {
        method: "POST",
        headers: {
        ...
        .then((data) => {
          if (data) {
            ...
            //if (data["Vehicle"]) alert(data["Vehicle"]);
          }
        })
        .catch((error) => {
  ...
  const handleAddCarDetails = (event) => {
    event.preventDefault();
    ...
    if (
      ...
      tax != "" &&
      engine != "" &&
      regex_digits.test(mileage) &&
      regex_digits.test(seats) &&
      regex_digits.test(keys)
    ) {
      const jsonData = {
        vin: vin,
        price: price,
      ...
      const url = "http://localhost:5000/insertCarDetails";
      fetch(url, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="managerAddCar">
      ...
      <Row className="profileRow">
          ...
          <div className="searchBarSale">
            <Form onSubmit={handleVinCheck}>
              ...
              <InputGroup className="mb-3">
                ...
                <Form.Control
                  id="managerAddIndivCarVin"
                  aria-label="Default"
                  aria-describedby="inputGroup-sizing-default"
                  value={vin_check}
                  onChange={(e) => setVinCheck(e.target.value)}
                />
              </InputGroup>
              ...
              <Button
                ...
                id="managerAddIndivCarSubmitButton"
              >
                ADD MODEL
              </Button>
            </Form>
        ...
        <Col className="middleCol">
          <Form onSubmit={handleAddCarDetails}>
            ...
            <Form.Group as={Col} className="textBar">
              ...
              <Form.Control
                className="inputBar"
                placeholder="Input EXTERIOR COLOR"
                type="text"
                value={ext_color}
            ...
            <Form.Group as={Col} className="textBar">
              ...
              <Form.Control
                className="inputBar"
                placeholder="Input INTERIOR COLOR"
                type="text"
                value={int_color}
